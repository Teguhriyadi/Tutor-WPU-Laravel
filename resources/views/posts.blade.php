...
<div class="card mb-3">
    ...
    <div class="card-body text-center">
        <h3 class="card-title">
            <a href="{{ url('/posts') }}/{{ $posts[0]->slug }}" class="text-decoration-none text-dark">
                {{ $posts[0]->title }}
            </a>
        </h3>
        <p>
            <small class="text-muted">
                By <a href="{{ url('/posts?author=') }}{{ $posts[0]->author->username }}" class="text-decoration-none">{{ $posts[0]->author->name }}</a> in
        ...
        <a href="{{ url('/posts') }}/{{ $posts[0]->slug }}" class="text-decoration-none btn btn-primary">
            Read More...
        </a>
    </div>
</div>
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="position-absolute bg-dark px-3 py-2 text-white" style="background-color: rgba(0, 0, 0, 0.7)">
                    <a href="{{ url('/posts?category=') }}{{ $post->category->slug }}" class="text-white text-decoration-none">
                        {{ $post->category->name }}
                    </a>
                </div>
                ...
                <div class="card-body">
                    ...
                    <p>
                        <small class="text-muted">
                            By <a href="{{ url('/posts?author=') }}{{ $post->author->username }}" class="text-decoration-none">{{ $post->author->name }}</a>
